(ns address_book.views.address_book_layout
...
(defn common-layout [& body]
  (html5
    ...
    [:body
     [:div#wrapper
      [:h1#content-title "Contatos"]
      [:div#contacts
       [:div.contact.header
        [:div.column-1
         [:i.icon-user.icon-style] " Nome"]
        [:div.column-2
         [:i.icon-phone.icon-style] " Telefone"]
        [:div.column-3
  ...
  (html
    [:div.contact
     [:form {:action "/post" :method "post"}
      [:div.column-1
       [:input#name-input {:type "text" :name "name" :placeholder "Nome"}]]
      [:div.column-2
       [:input#phone-input {:type "text" :name "phone" :placeholder "Telefone"}]]
      [:div.column-3
       [:input#email-input {:type "text" :name "email" :placeholder "Email"}]]
      [:button.button.add {:type "submit"} "Salvar "]]
     [:div.clear-row]]))
